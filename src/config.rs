...
impl Config {
    pub fn from_env() -> AppResult<Self> {
        ...
        let ws_path = env::var("AGENT_WS_PATH").unwrap_or_else(|_| "/ws".to_string());
        let api_token = env::var("AGENT_API_TOKEN").ok().filter(|token| !token.trim().is_empty());
...
fn parse_allowed_roots(value: Option<std::ffi::OsString>) -> AppResult<Vec<PathBuf>> {
    let roots = match value {
        ...
        _ => vec![absolutize_path(&env::var("HOME").unwrap_or_else(|_| ".".to_string()))?],
    };
